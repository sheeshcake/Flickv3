...
const hero = async () => {
    const heromovie = await fetch(
        `${tmdb_api}discover/movie?api_key=${api_key}&language=${'en-US'}&page=1`,
    );
    ...
    const herotv = await fetch(
        `${tmdb_api}discover/tv?api_key=${api_key}&language=${'en-US'}&page=1`,
    );
...
const popular_movie = async () => {
    const popularmovie = await fetch(
        `${tmdb_api}movie/top_rated?api_key=${api_key}&language=${'en-US'}&sort_by=popularity.desc&page=1`,
    );
...
const horror_movie = async () => {
    const horrormovie = await fetch(
        `${tmdb_api}discover/movie?api_key=${api_key}&language=${'en-US'}&with_genres=27&page=1`,
    );
...
const action_movie = async () => {
    const actionmovie = await fetch(
        `${tmdb_api}discover/movie?api_key=${api_key}&language=${'en-US'}&with_genres=28&page=1`,
    );
...
const comedy_movie = async () => {
    const comedymovie = await fetch(
        `${tmdb_api}discover/movie?api_key=${api_key}&language=${'en-US'}&with_genres=35&page=1`,
    );
...
const romance_movie = async () => {
    const romancemovie = await fetch(
        `${tmdb_api}discover/movie?api_key=${api_key}&language=${'en-US'}&with_genres=10749&page=1`,
    );
...
const get_seasons_and_episodes = async (id) => {
    const tv_details = await fetch(
        `${tmdb_api}tv/${id}?api_key=${api_key}&language=${'en-US'}`,
    );
    const tvdata = await tv_details.json();
    let tv_details_data = tvdata.seasons.map(season => {
        return {
            id: season.id,
    ...
    for (let season of tv_details_data) {
        const episodes = await fetch(
            `${tmdb_api}tv/${id}/season/${season.season_number}?api_key=${api_key}&language=${'en-US'}`,
        );
        const episodata = await episodes.json();
        season.episodes = episodata.episodes;
    }
    return tv_details_data;
};

const get_episodes = async (id, season) => {
    const episodes = await fetch(
        `${tmdb_api}tv/${id}/season/${season || 1
...
const popular_tv = async () => {
    const populartv = await fetch(
        `${tmdb_api}discover/tv?api_key=${api_key}&language=${'en-US'}&sort_by=popularity.desc&page=1`,
    );
...
const search_movie = async search => {
    const searchmovie = await fetch(
        `${tmdb_api}search/movie?api_key=${api_key}&language=${'en-US'}&query=${search}&page=1`,
    );
...
const search_tv = async search => {
    const searchtv = await fetch(
        `${tmdb_api}search/tv?api_key=${api_key}&language=${'en-US'}&query=${search}&page=1`,
    );
...
const get_movie_details = async (id, type) => {
    const details = await fetch(`${tmdb_api}${type}/${id}?api_key=${api_key}&language=${'en-US'}`);
    const trailerResponse = await fetch(
        `${tmdb_api}${type}/${id}/videos?api_key=${api_key}&language=${'en-US'}`,
    );
    const data = await details.json();
    const trailerData = await trailerResponse.json();
    const recommendedResponse = await fetch(
        `${tmdb_api}${type}/${id}/recommendations?api_key=${api_key}&language=${'en-US'}&page=1`,
    );
    ...
    let details_data = {
        ...
        // embedded youtube link automatically play the video
        trailer: trailerData.results.length
            ? `https://www.youtube.com/embed/${trailerData.results[0].key}?autoplay=1&controls=0&mute=1&loop=1`
            : null,
        ...
        genres: data.genres || [],
        recommended: recommendedData.results.map(item => ({
            id: item.id,
            type: type,
